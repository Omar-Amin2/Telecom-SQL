...
EXECUTE Account_Payment_Points '01234567890'
...
SELECT * FROM Cashback_Wallet_Customer('123456789')
...
Select * FROM dbo.Subscribed_plans_5_Months('01234567891') 
...
EXEC [Payment_wallet_cashback] @mobile_num = '12345678901', @payment_id = 1, @benefit_id = 1;
...
BEGIN
    IF (SELECT v.points 
        FROM Voucher v 
        WHERE v.voucherID = @voucher_id AND v.expiry_date > CURRENT_TIMESTAMP) 
        <= (SELECT a.points 
    ...
    ELSE
    BEGIN
        RAISERROR('Not enough points to redeem voucher or voucher is expired.', 16, 1);
    END
END;
...
update customer_account SET points = 1000 WHERE mobileNo = '01234567891'
...
Select * from Benefits
GO
CREATE PROCEDURE populateSampleData AS
BEGIN
    -- Insert into Customer_profile
    INSERT INTO Customer_profile (nationalID, first_name, last_name, email, address, date_of_birth)
    VALUES 
        (123456789, 'John', 'Doe', 'john.doe@example.com', '123 Elm Street', '1990-01-01'),
        (987654321, 'Jane', 'Smith', 'jane.smith@example.com', '456 Oak Avenue', '1985-05-15'),
        (555666777, 'Alice', 'Brown', 'alice.brown@example.com', '789 Pine Road', '1992-07-08'),
        (111222333, 'Bob', 'Green', 'bob.green@example.com', '321 Birch Blvd', '1988-11-23'),
        (444555666, 'Emma', 'White', 'emma.white@example.com', '654 Maple Drive', '1995-02-14');

    -- Insert into Customer_Account
    INSERT INTO Customer_Account (mobileNo, pass, balance, account_type, start_date, status, points, nationalID)
    VALUES 
        ('01234567891', 'password123', 100.0, 'prepaid', '2024-01-01', 'active', 10, 123456789),
        ('09876543210', 'securepass', 50.0, 'postpaid', '2024-01-15', 'onhold', 20, 987654321),
        ('01324567890', 'mypassword', 80.0, 'pay-as-you-go', '2024-02-01', 'active', 15, 555666777),
        ('09182736455', 'pass1234', 120.0, 'postpaid', '2024-03-10', 'onhold', 25, 111222333),
        ('01928374651', 'abcpass', 90.0, 'prepaid', '2024-04-01', 'active', 30, 444555666);

    -- Insert into Service_Plan
    INSERT INTO Service_Plan (SMS_offered, minutes_offered, data_offered, name, price, description)
    VALUES 
        (500, 100, 5, 'Basic Plan', 100, 'Affordable monthly plan'),
        (1000, 200, 10, 'Premium Plan', 200, 'Best value for heavy users'),
        (1500, 300, 20, 'Pro Plan', 300, 'Designed for professionals'),
        (2000, 500, 50, 'Elite Plan', 500, 'Exclusive features for high-end users'),
        (250, 50, 2, 'Starter Plan', 50, 'Entry-level plan for beginners');

    -- Insert into Subscription
    INSERT INTO Subscription (mobileNo, planID, subscription_date, status)
    VALUES 
        ('01234567891', 1, '2024-01-10', 'active'),
        ('09876543210', 2, '2024-01-20', 'onhold'),
        ('01324567890', 3, '2024-02-05', 'active'),
        ('09182736455', 4, '2024-03-15', 'onhold'),
        ('01928374651', 5, '2024-04-10', 'active');

    -- Insert into Plan_Usage
    INSERT INTO Plan_Usage (start_date, end_date, data_consumption, minutes_used, SMS_sent, mobileNo, planID)
    VALUES 
        ('2024-01-01', '2024-01-31', 3, 50, 200, '01234567891', 1),
        ('2024-01-01', '2024-01-31', 8, 150, 900, '09876543210', 2),
        ('2024-02-01', '2024-02-28', 5, 100, 300, '01324567890', 3),
        ('2024-03-01', '2024-03-31', 10, 200, 400, '09182736455', 4),
        ('2024-04-01', '2024-04-30', 2, 30, 100, '01928374651', 5);

    -- Insert into Payment
    INSERT INTO Payment (amount, date_of_payment, payment_method, status, mobileNo)
    VALUES 
        (100.0, '2024-01-05', 'cash', 'successful', '01234567891'),
        (200.0, '2024-01-25', 'credit', 'pending', '09876543210'),
        (50.0, '2024-02-15', 'cash', 'successful', '01324567890'),
        (300.0, '2024-03-20', 'credit', 'rejected', '09182736455'),
        (400.0, '2024-04-10', 'cash', 'successful', '01928374651');
    ...
    VALUES 
        (500.0, 'USD', '2024-01-20', 123456789, '01234567891'),
        (200.0, 'USD', '2024-01-25', 987654321, '09876543210'),
        (300.0, 'EUR', '2024-02-20', 555666777, '01324567890'),
        (400.0, 'GBP', '2024-03-25', 111222333, '09182736455'),
        (150.0, 'USD', '2024-04-10', 444555666, '01928374651');
    ...
    VALUES 
        ('Free SMS Pack', '2024-02-15', 'active', '01234567891'),
        ('Extra Data', '2024-03-01', 'expired', '09876543210'),
        ('Weekend Offer', '2024-04-10', 'active', '01324567890'),
        ('Night Bundle', '2024-05-15', 'expired', '09182736455'),
        ('Holiday Bonus', '2024-06-01', 'active', '01928374651');
    ...
    VALUES 
        ...
        ('SuperBooks', 'Books'),
        ('GadgetHub', 'Gadgets'),
        ('FashionSquare', 'Clothing'),
        ('HomeEssentials', 'Home Goods');

    -- Insert into PhysicalShop
    INSERT INTO Physical_shop (shopID, address, working_hours)
    VALUES 
        (1, '789 Tech Plaza', '9AM-9PM'),
        (2, '123 Book Avenue', '10AM-8PM'),
        (3, '456 Gadget Road', '11AM-10PM'),
        (4, '789 Fashion Street', '10AM-6PM'),
        (5, '123 Home Blvd', '8AM-5PM');

    -- Insert into E_shop
    INSERT INTO E_shop (shopID, URL, rating)
    VALUES 
        (1, 'www.electromart.com', 5),
        (2, 'www.superbooks.com', 4),
        (3, 'www.gadgethub.com', 4),
        (4, 'www.fashionsquare.com', 3),
        (5, 'www.homeessentials.com', 5);

    -- Insert into Voucher
    INSERT INTO Voucher (value, expiry_date, points, mobileNo, shopID, redeem_date)
    VALUES 
        (50, '2024-12-31', 500, NULL, 1, NULL),
        (100, '2024-11-30', 1000, NULL, 2, NULL),
        (75, '2024-10-31', 750, NULL, 3, NULL),
        (125, '2024-09-30', 1250, NULL, 4, NULL),
        (150, '2024-08-31', 1500, NULL, 5, NULL);
    ...
    VALUES 
        ('01234567891', 'Network issue', 2, 'Open'),
        ('09876543210', 'Billing query', 1, 'Resolved'),
        ('01324567890', 'App Crash', 3, 'In Progress'),
        ('09182736455', 'Payment Failed', 2, 'Open'),
        ('01928374651', 'Service Downtime', 4, 'Resolved');
END;
GO

SELECT * FROM process_payment

--EXEC populateSampleData